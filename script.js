...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // New Premium Unlock Modal elements
    const modalPremiumUnlock = document.getElementById('modal-premium-unlock');
    const openPremiumUnlockModalBtn = document.getElementById('openPremiumUnlockModalBtn');
    const premiumCodeInput = document.getElementById('premiumCodeInput');
    const submitPremiumCodeBtn = document.getElementById('submitPremiumCodeBtn');
    const premiumUnlockMessage = document.getElementById('premiumUnlockMessage');
    ...
    // --- Premium Feature Unlock System Variables and Constants ---
    // The secret key for generating the monthly code. Keep this secure.
    // In a real application, this might be fetched from a server or environment variable.
    const MONTHLY_CODE_SECRET = 'D&DTrackerSecretKey2025!'; // This should be a strong, random string.
    let hasPremiumAccess = false; // Tracks if premium features are currently unlocked
...
Once you have provided the JSON - Ask the player to return to the Initiative tracker to upload it and thank them for being a patreon/jimpeccable Supporter.\n\n`;
    ...
    function toggleDeathSave(combatantId, type, index) {
        const combatant = combatants.find(c => c.id === combatantId);
        if (combatant && combatant.isPC && combatant.currentHp <= 0 && combatant.deathSaves) { // Only allow death saves if HP is 0 or less
            if (type === 'success') {
            ...
            renderCombatants();
            saveEncounter();
        } else if (combatant && combatant.isPC && combatant.currentHp > 0) {
            openMessageModal('Not Dying', `${combatant.name} is not at 0 HP. Death saves only apply when dying.`);
        }
    }
    ...
    function openModal(modalElement) {
        ...
        } else if (modalElement.id === 'modal-premium-unlock') {
            premiumCodeInput.value = '';
            premiumUnlockMessage.textContent = '';
            premiumUnlockMessage.className = 'message-area';
...
/**
 * Updates UI elements based on premium feature access status.
 * Applies/removes 'premium-locked-feature' class.
 */
function updateFeatureAccessUI() {
    const lockedElements = document.querySelectorAll('.premium-locked-feature');
    if (hasPremiumAccess) {
        lockedElements.forEach(el => el.classList.remove('premium-locked-feature'));
        ...
        premiumStatusDiv.textContent = 'Premium Status: Unlocked!';
        premiumStatusDiv.style.color = 'var(--color-green)';
        console.log('Premium features unlocked!');
    } else {
        lockedElements.forEach(el => el.classList.add('premium-locked-feature'));
        ...
        premiumStatusDiv.textContent = 'Premium Status: Locked';
        premiumStatusDiv.style.color = 'var(--color-red)';
        console.log('Premium features locked.');
    }
}

/**
 * Handles the submission of the premium unlock code.
 */
async function handlePremiumCodeSubmission() {
    const enteredCode = premiumCodeInput.value.trim();
    if (enteredCode.length !== 8) {
        premiumUnlockMessage.textContent = 'Code must be 8 characters long.';
        premiumUnlockMessage.className = 'message-area error';
        return;
    }

    const isValid = await checkMonthlyCode(enteredCode);

    if (isValid) {
        hasPremiumAccess = true;
        const date = new Date();
        const monthYear = `${date.getFullYear()}-${(date.getMonth() + 1).toString().padStart(2, '0')}`;
        sessionStorage.setItem('premiumAccessStatus', 'true');
        sessionStorage.setItem('premiumAccessMonthYear', monthYear);

        premiumUnlockMessage.textContent = 'Features unlocked successfully!';
        premiumUnlockMessage.className = 'message-area success';
        updateFeatureAccessUI();
        setTimeout(() => {
            closeModal(modalPremiumUnlock);
            openMessageModal('Success', 'Premium features unlocked for this month!');
        }, 1000); // Close modal after a short delay
    } else {
        hasPremiumAccess = false;
        premiumUnlockMessage.textContent = 'Incorrect code. Please try again.';
        premiumUnlockMessage.className = 'message-area error';
        updateFeatureAccessUI();
    }
}
    ...
    // Open Premium Unlock Modal Button
    openPremiumUnlockModalBtn.addEventListener('click', () => openModal(modalPremiumUnlock));
    ...
    window.addEventListener('click', (event) => {
        ...
        } else if (event.target === modalPremiumUnlock) {
            closeModal(modalPremiumUnlock);
        }
    });
    ...
    savePartyFileBtn.addEventListener('click', () => {
        if (!hasPremiumAccess) {
            openMessageModal('Premium Feature', 'This feature is available for premium users only. Please unlock features with the monthly code.');
            return;
        }
        if (combatants.length === 0) {
    ...
    // Premium Unlock Code Submission
    submitPremiumCodeBtn.addEventListener('click', handlePremiumCodeSubmission);
    premiumCodeInput.addEventListener('keydown', (e) => {
        if (e.key === 'Enter') {
            handlePremiumCodeSubmission();
        }
    });
    ...
    // Check premium access on load
    const savedAccessStatus = sessionStorage.getItem('premiumAccessStatus');
    const savedAccessMonthYear = sessionStorage.getItem('premiumAccessMonthYear');
    const currentDate = new Date();
    const currentMonthYear = `${currentDate.getFullYear()}-${(currentDate.getMonth() + 1).toString().padStart(2, '0')}`;

    if (savedAccessStatus === 'true' && savedAccessMonthYear === currentMonthYear) {
        hasPremiumAccess = true;
    } else {
        hasPremiumAccess = false;
        sessionStorage.removeItem('premiumAccessStatus');
        sessionStorage.removeItem('premiumAccessMonthYear');
    }
    updateFeatureAccessUI();
});
